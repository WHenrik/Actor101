  ...
  override def main(args: Array[String]): Unit = {
    ...
    */
    ...
    args.sliding(2, 2).toList.collect {
      ...
      case Array("--cores", argCores: String) => nbCores = argCores.toInt
    }
    // Create a SparkSession to work with Spark
    val sparkBuilder = SparkSession
      .builder()
      .appName("SparkTutorial")
      .master(s"local[$nbCores]") // local, with 4 worker cores
...
*/
    ...
    //val inputs = List("lineitem", "region", "nation", "supplier", "customer", "part")
    //val inputs = List("region", "nation", "supphlier", "customer")
